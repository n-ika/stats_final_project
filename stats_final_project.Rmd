...
alpha05 <- statistical_power(geom_subset, 0.05)
alpha001 <- statistical_power(geom_subset, 0.0001)
...
for (i in 1:9999) {
          permuted_geom <- dplyr::data_frame(
                                   ...
                                   corr_resp_mfcc_EN$user_corr, 
                                   (dim(corr_resp_mfcc)[1]/2),
                                   replace = TRUE),
                                   permuted_french = sample(
                                   corr_resp_mfcc_FR$user_corr, 
                                   (dim(corr_resp_mfcc)[1]/2),
                                   replace = TRUE))
...
When checking the mean difference with permutation, the observed mean difference is at the center of the distribution of the permuted mean differences. The number of the observed mean difference is quite small and close to zero (`-0.056`). It therefore makes sense that the permutation result gives a gaussian distribution with the observed mean difference centered at it's mean.


This and the previous results show that we can reject the H0 which states that the acoustic predictions have no influence on human responses. We also see that the observed mean difference between the two groups separated by native language is close to 0. Globally, we can conclude that acoustic distances have influence on perception and that there is no significant difference between the two language groups.

But to me it is interesting to know which pair specifically was influenced by the distance more than another and was problematic for a language group and if so, are the responses accordingly correct or wrong.


## 4) ORDER STATISTICS OF WORSE DISCRIMINATED SOUND PAIRS

To understand if the less well discriminated pairs (e.g., [θ] - [f]) are more influenced by the acoustics than the other pairs, I will check the order of the pairs by their order of discrimination rate.
...
*****
Simulate the sampling distribution of the coefficient (and also the t-statistic)
...
m_H0_FR <- glm(user_corr ~ correct_resp,
           data=corr_resp_mfcc_FR)
boot_m_H0_FR <- car::Boot(logit_m_FR, f=logit_coefficient,
                       r=9999, method="case")


m_H0_EN <- glm(user_corr ~ correct_resp,
           data=corr_resp_mfcc_EN)
boot_m_H0_EN <- car::Boot(logit_m_EN, f=logit_coefficient,
                       r=9999, method="case")
```

Histogram of bootstrapped interaction coefficient under H0,
 that coefficient is 0.
